...
#pragma comment(lib, "gstbase-0.10.lib")
//#pragma comment(lib, "orc-0.4.lib")


#include "gstvideotestsrc.h"
#include "gstvideotestsrcorc-dist.h"
...
void
gst_video_test_src_pipe (GstVideoTestSrc * v, GstBuffer *outbuf, int *w, int *h)
{
  int i;
  int j;
  paintinfo pi = { NULL, };
  ...
  void *dest = GST_BUFFER_DATA (outbuf);

  static FILE *fin = NULL;
  static void * dummy_buf = NULL;

  size = GST_BUFFER_SIZE (outbuf);

  if (fin == NULL) {
	  fin = fopen(TEXT("\\\\.\\pipe\\test"),"rb");
  }
  if (fin == NULL)
	  return;

  read = fread(&hdr,1,sizeof(frame_header_t),fin);

  if (read < sizeof(frame_header_t))
	  goto recon;

  if (hdr.magic != 0xf00dbeaf)
	  goto recon;

  if (hdr.info.FrameWidth != *w || hdr.info.FrameHeight != *h) {
	  *w = hdr.info.FrameWidth;
	  *h = hdr.info.FrameHeight;
	  return;
  }


  frame_sz = hdr.info.FrameHeight * hdr.info.LineLength;
  
  if (frame_sz != size && !dummy_buf)
	  dummy_buf = malloc(frame_sz);

  read = fread(
	  frame_sz != size ? dummy_buf : dest,
	  1,frame_sz,fin);
...
static GstBuffer *
reallocate(GstPushSrc * psrc, gint width, gint height, GstBuffer *outbuf) {
  ...
  newsize = gst_video_test_src_get_size (src, width, height);

  GST_LOG_OBJECT (src,
      "creating buffer of %lu bytes with %dx%d image for frame %d", newsize,
      width, height, (gint) src->n_frames);
...
gst_video_test_src_create (GstPushSrc * psrc, GstBuffer ** buffer)
{
  ...
  outbuf = reallocate(psrc, src->width, src->height, outbuf);
  if (outbuf == NULL)
	  goto no_buffer;
...
// ------------------------ читаем картинку из пайпа ---------------------------
  w = src->width;
  h = src->height;

  gst_video_test_src_pipe(src, outbuf,&w, &h);

  if (w != src->width || h != src->height) {

	  //outbuf = reallocate(psrc, src->width, src->height, outbuf);
	//  outbuf = reallocate(psrc,w,h, outbuf);
	  //if (outbuf == NULL)
		 // goto no_buffer;
	 // gst_video_test_src_pipe(src, (void *) GST_BUFFER_DATA (outbuf),&w, &h);
  }
...
plugin_init (GstPlugin * plugin)
{
  ...
  GST_DEBUG_CATEGORY_INIT (video_test_src_debug, "pipesrc", 0,
      "DScaler Source");

  return gst_element_register (plugin, "pipesrc", GST_RANK_NONE,
      GST_TYPE_VIDEO_TEST_SRC);
}
#define PACKAGE "my_pkg"
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    "pipesrc",
    "Creates a DScaler pipe video stream",
    plugin_init, "v1", "GPL", "GStreamer", "www.google.com")
